"""Integration tests for the collections surface (sync REST).
...
validation in test_client.py. See docs/migration/v10-migration.md,
...
def test_collection_from_serverless_raises_error_rest(client: Pinecone) -> None:
    """Creating a collection from a serverless index raises ApiError(400).
    ...
    This verifies that the SDK reaches POST /collections and surfaces the
    refusal as ApiError with status_code=400 rather than as a transport or
    routing error.
    ...
    try:
        ...
        with pytest.raises(ApiError) as exc_info:
            client.collections.create(name=col_name, source=index_name)
