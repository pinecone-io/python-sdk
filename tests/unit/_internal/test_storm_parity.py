...
async (DX-0167), and bulk-upsert (DX-0168) storm tests. Each test writes a
JSON metrics file; this module reads all three and asserts they are within
acceptable bounds of each other.

All comparison tests are marked xfail until DX-0167 and DX-0168 land.
"""
...
_BULK_PATH = _DIR / "_storm_parity_metrics_bulk.json"
...
def test_dispersion_widths_within_2x() -> None:
    ...
    assert bulk_w <= sync_w * 2.0, f"bulk dispersion {bulk_w:.3f} > 2x sync {sync_w:.3f}"
...
def test_amplifications_within_1_5x() -> None:
    ...
    bulk_a = float(bulk_m["request_amplification"])  # type: ignore[arg-type]
    ...
    assert bulk_a <= sync_a * 1.5, f"bulk amp {bulk_a:.3f} > 1.5x sync {sync_a:.3f}"
    assert sync_a <= bulk_a * 1.5, f"sync amp {sync_a:.3f} > 1.5x bulk {bulk_a:.3f}"
