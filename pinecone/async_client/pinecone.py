    ...
    async def configure_index(
        ...
        **legacy_kwargs: Any,
        ...
        .. versionchanged:: 10.0
           ...
           ``deployment=``) and returns the updated :class:`IndexModel`
           instead of ``None``. ``replicas=``/``pod_type=``/
           ``serverless_read_capacity=`` still work as deprecated sugar;
           ``embed=`` and ``spec=`` raise a
